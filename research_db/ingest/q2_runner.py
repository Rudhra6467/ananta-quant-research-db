...
EXPECTED_HOURS = 2184
# Stop if we have any trade in the last minute of Q2, or the cursor does not move.
END_SLACK = 60
...
def open_db(db: Path) -> sqlite3.Connection:
    ...
    return conn
def flush_bars(conn, pair, ticks):
    inst = PAIR_MAP[pair]
    ...
    conn.commit()
    return len(bars)
def reached_end(last_ts: float) -> bool:
    return last_ts >= (Q2_END_UNIX - END_SLACK)
def run_pair(pair: str, db: Path, sleep_s: float = 1.2, max_pages: int | None = None) -> dict:
    ...
    ticks = []
    stall = 0
    done = reached_end(last_ts) or float(since) >= Q2_END_UNIX
    while not done:
        ...
        conn.commit()
        print({"pair": pair, "pages": pages, "last_ts": last_ts, "stall": stall}, flush=True)
        if reached_end(last_ts) or stall >= 2 or float(since) >= Q2_END_UNIX:
            done = True
            break
        time.sleep(sleep_s)
    n_bars = flush_bars(conn, pair, ticks)
    inst = PAIR_MAP[pair]
    conn.close()
